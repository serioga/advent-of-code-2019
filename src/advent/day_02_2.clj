...
(def target-output 19690720)


(defn test-result
  [[noun verb output]]
  (when (= output target-output)
    (as-answer noun verb)))


(defn solve
  []
  (let [idx-xs (range (alength ^ints part1/input))]
    (->> (for [noun idx-xs
               verb idx-xs]
           [noun verb (try-run noun verb)])
      (some test-result))))


(comment
  (t/test-var #'as-answer)
  (time (solve)))
