...
Finally, some notes:
  ...
  - It is important to remember that the instruction pointer should increase by the number of values
    in the instruction after the instruction finishes. Because of the new instructions, this amount
...
(defn run-output!
  "Execute output operation and return next address to operate on."
  ^long [^ints program, ^long addr, *io]
...
(defn run-program
  ...
  {:test (fn []
           (t/is (== 3500 (run-program [1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50] (atom 1))))
           (t/is (== 2 (run-program [1, 0, 0, 0, 99] (atom 1))))
