...
(defn solve
  ...
  (->> (layers input w, h)
    (map frequencies)
    (reduce (fn [a b] (let [na (get a 0 0)
                            nb (get b 0 0)]
                        (if (<= na nb) a b))))
    ((fn [m] (* (get m 1 0) (get m 2 0))))))


(def input
  (-> "advent/day_08_input.txt"
    (io/resource)
    (slurp)
    (data->image)))
...
(comment
  (time
    (solve input 25 6))
  (do input)
  (data->image "123456789012")
  (layers (data->image "123456789012") 3 2)
  (solve (data->image "123456789012") 3 1))
