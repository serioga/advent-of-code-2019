...
looking at the first integer (called position 0). Here, you will find an opcode - either 1, 2, or
99. The opcode indicates what to do; for example, 99 means that the program is finished and should
...
(defn run-op!
  "Execute operation and return next index"
  ^long [^ints program!, ^long idx, op]
  (let [a (val-of-idx program! (+ idx 1))
        b (val-of-idx program! (+ idx 2))
        res (int (op a b))
        res-idx (aget program! (+ idx 3))]
    (aset program! res-idx res)
    (+ idx 4)))
...
(defn clone
  ^ints
  [program]
  (aclone (int-array program)))


(defn run-program
  "Execute program and return value at position 0"
  {:test (fn []
           (t/is (== 3500 (run-program [1,9,10,3,2,3,11,0,99,30,40,50])))
           (t/is (== 2 (run-program [1, 0, 0, 0, 99])))
           (t/is (== 2 (run-program [2, 3, 0, 3, 99])))
           (t/is (== 2 (run-program [2, 4, 4, 5, 99, 0])))
           (t/is (== 30 (run-program [1, 1, 1, 4, 99, 5, 6, 0, 99]))))}
  [program]
  (loop [program! (clone program)
         idx 0]
    (case (aget program! idx)
...
(comment
  ...
  (clojure.pprint/pprint (init-program input)))
